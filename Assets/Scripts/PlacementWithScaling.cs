...
public class PlacementWithScaling : MonoBehaviour
{
    [SerializeField]
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                ...
                {                    
                    ...
                    if (lastSelectedObject == null)
                    {
                        lastSelectedObject = Instantiate(placedPrefab, hitPose.position, new Quaternion(0, 0.9f, 0, 0)).GetComponent<PlacementObject>();
                        Vector3 rot = lastSelectedObject.transform.rotation.eulerAngles;
                        rot = new Vector3(rot.x, rot.y + 180, rot.z);
                        lastSelectedObject.transform.rotation = Quaternion.Euler(rot);
                    }
                }
                else
                {
